...
def uniform_cost_search(gr, s, g):
    ...
    while len(frontier.queue) > 0:
        ...
        for act in actions:
            ...
            try:
                frontier.put((int(gr[next_c[0]][next_c][0]), next_c))
            except Exception as error:
...
def print_all_types(gr, start_cell, goal_cell):
    ...
    # Where I got this: https://stackoverflow.com/questions/48053979/print-2-lists-side-by-side
    # table = "\n".join("{} {}".format(grid_bfs, grid_dfs) for x, y in zip(grid_bfs, grid_dfs))

    print_table(grid_bfs, grid_dfs, grid_ucs, reached_bfs, reached_dfs, reached_ucs, path_bfs, path_dfs, path_ucs)


def print_table(g1, g2, g3, r1, r2, r3, p1, p2, p3):
    ...
    g4 = [[gg1[i] + gg2[i] + gg3[i]] for i in range(len(g1))]
    # print key
    print("-" * len(gg1[0]) * 3)
    print("Key:")
    ...
    spacer = (" " * len(g1[0]))
    print(spacer + "BFS" + " " + spacer * 2 + "DFS" + " " + spacer * 2 + "UCS")
    # s1 = "V: " + str(len(r1))
    # if len(s1) < 3:
...
if __name__ == '__main__':
    ...
    usr_input = input("Select Algorithm ('B' for BFS, 'D' for DFS, 'U' for UCS): ")

    # make sure user enters a valid input.
    while usr_input.lower() != "b" and usr_input.lower() != "d" and usr_input.lower() != "u":
